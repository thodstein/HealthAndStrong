...
const EXTENDED_SPLITS: Record<string, { name: string; desc: string; groupsPerDay: string[][]; minDays: number; maxDays: number; level: string[] }> = {
  ...
  strength_4: { name: 'Силовой 4 дня', desc: 'Присед/Жим/Тяга/ОФП. Для пауэрлифтеров.', groupsPerDay: [['legs','core'],['chest','shoulders'],['back','arms'],['legs','shoulders']], minDays: 4, maxDays: 4, level: ['intermediate','advanced','enhanced'] },
...
export const MESOCYCLE_PARAMS: Record<MesocycleType, { volumeMultiplier: number; rirRange: [number, number]; rpeTarget: number; description: string }> = {
  ...
  intensification: { volumeMultiplier: 0.85, rirRange: [1, 2], rpeTarget: 8.5, description: 'Интенсификация. Объём снижается, интенсивность растёт. Рабочие веса ↑, повторения ↓.' },
  peaking: { volumeMultiplier: 0.7, rirRange: [0, 1], rpeTarget: 9.5, description: 'Пиковый блок. Максимальные веса, минимальный объём. Подход к 1-3 ПМ.' },
  deload: { volumeMultiplier: 0.5, rirRange: [3, 5], rpeTarget: 6, description: 'Разгрузка. 50-60% объёма, RIR 3-5. Восстановление СНС, сверхкомпенсация.' },
