...
export const LabsScreen: React.FC = () => {
  ...
  const labsBySystem: Record<string, string[]> = useMemo(() => {
  ...
  const labIndexDetails: Record<string, LabIndexDetail> = useMemo(() => {
        ...
        <div>
          ...
          {labRisks && (
            <div className="card" style={{ marginTop: 8 }}>
              ...
              {/* Systems breakdown */}
              {Object.entries(labRisks.systemBreakdown as Record<string, { raw: number; net: number }>).filter(([_, v]) => v.net > 0).length > 0 && (
                <div style={{ marginTop: 4 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, marginBottom: 4, color: 'var(--text-dim)' }}>По системам:</div>
                  <div style={{ display: 'grid', gap: 2 }}>
                    {Object.entries(labRisks.systemBreakdown as Record<string, { raw: number; net: number }>)
                      .filter(([_, v]) => v.net > 0)
                      .sort(([_, a], [__, b]) => b.net - a.net)
                      .slice(0, 8)
                      .map(([sys, val]: [string, { raw: number; net: number }]) => (
